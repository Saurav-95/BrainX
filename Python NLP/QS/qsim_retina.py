...
class Question_Similarity:
	...
	def func2(self, tid, num_threads, distance):
		for i in range(self.train_size+int(tid*(self.q_total-self.train_size)/num_threads)+1, min(self.q_total,self.train_size+int((tid+1)*(self.q_total-self.train_size)/num_threads))):
			print(tid,i)
			distance[tid] = self.compare(self.q_actual[i])
	...
	def nearest_neighbour(self, fname) :
		...
		"""
		qout = open(fname,"w")
		self.q_total = 5000 #len(self.q_list)
		num_threads = 10
		threads = [None] * num_threads
		distance = [None] * num_threads
		for i in range(num_threads):
			threads[i] = Thread(target=self.func2, args=(i,num_threads,distance))
			threads[i].start()
		...
		for i in range(self.train_size+1,self.q_total):
			...
			for j in range(self.n_no):
				qout.write("NN %d (%f) --- " % (j+1, distance[int((i-self.train_size-1)/num_threads)][j][1])+self.q_actual[distance[int((i-self.train_size-1)/num_threads)][j][0]])
		qout.close()		
	
if __name__ == "__main__" :
	#logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
	qs = Question_Similarity(4900, 10)
